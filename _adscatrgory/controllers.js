...
async function addCategory(req, res) {
  ...
  const newCategory = new Category({ _id, nameCategory });
  newCategory.save(async (err, category) => {    
        if (err) {
            switch (err.code) {
                case 11000:
                    res.status(400).json({ "message": "Name of category in use", "err": err });
                    break;
...
async function updateCategory(req, res) {
  ...
  const prop = req.body;
  Category.findByIdAndUpdate(id, { $set: { ...prop } }, {returnDocument: 'after'})
    .exec((err, category) => {
      if (err) {
        res.status(500).json({ message: "Error occured" });
        return;
